...
def transmittance_coeff_calculation_function(uid, building_fields_table_df, julian_date_df, local_standard_time_df, global_transmittance_coeff_csvfile_location, global_solar_irradiance_geometry_csvfile_name, steps, meter_above_sea_level, angstrom_turbidity, saturated_vapor_pressure, relative_humidity, ambient_temperature, ozone_layer, start_time, stop_time):
    ...
    new_building_fields_tables_df               = pd.DataFrame((building_fields_table_df.values.tolist())*end_julian_date)
    ...
    new_global_transmittance_coeff_df = pd.concat([new_building_fields_tables_df, date_table_df, julian_date_table_df, sunrise_table_df, sunset_table_df, global_transmittance_coeff_df], axis=1)
