...
def global_solar_irradiance_calculation_funciton(attribute_table_csvfile_name, uid, building_fields_table_df, global_solar_irradiance_csvfile_location, global_solar_irradiation_csvfile_location, global_solar_building_analysis_csvfile_location, globla_solar_daily_irradiance_analysis_csvfile_location, global_solar_irradiance_geometry_csvfile_name, global_transmittance_coeff_csvfile_name, steps, barometric_pressure, angstrom_turbidity):
    ...
    diffuse_irradiance_from_mutiple_reflection_field_list = []
    diffuse_irradiance_from_mutiple_reflection_global_df = pd.DataFrame([])
    # Diffuse Irradistion from Mutiple Reflection Data Base
    diffuse_irradiation_from_mutiple_reflection_field_list = []
    diffuse_irradiation_from_mutiple_reflection_global_df = pd.DataFrame([])
    ...
    for julian_date in range (1, end_julian_date):
        ...
        diffuse_irradiance_from_mutiple_reflection_local_list = []
        diffuse_irradiation_from_mutiple_reflection_local_list = []
        ...
        for hour in range (start_time, stop_time+1):
            ...
            for minute in range(0, 60, steps):
                ...
                if julian_date == 1:
                    ...
                    diffuse_irradiance_from_mutiple_reflection_field_list.append(f"Diffuse Irradiance from Mutiple Reflection-{hour_time}:{minute_time}")
                    diffuse_irradiation_from_mutiple_reflection_field_list.append(f"Diffuse Irradiation from Mutiple Reflection-{hour_time}:{minute_time}")
                ...
                diffuse_irradiance_from_mutiple_reflection_local_list.append(diffuse_irradiance_from_mutiple_reflection)
                # Diffuse Irradiation on Horizontal Surface Calculation
                diffuse_irradiation, diffuse_irradiation_from_rayleigh_atmosphere, diffuse_irradiation_from_aerosol_scattering, diffuse_irradiation_from_mutiple_reflection = diffuse_irradiation_on_horizontal_surface_function(direct_normal_irradiation, rayleigh_transmittance_coeff, scattering_by_aerosol_transmittance_coeff, absorption_transmittance_coeff, barometric_pressure, angstrom_turbidity)
                ...
                diffuse_irradiation_from_mutiple_reflection_local_list.append(diffuse_irradiation_from_mutiple_reflection)
        ...
        diffuse_irradiance_from_mutiple_reflection_local_df = pd.DataFrame(diffuse_irradiance_from_mutiple_reflection_local_list)
        diffuse_irradiance_from_mutiple_reflection_global_df = pd.concat([diffuse_irradiance_from_mutiple_reflection_global_df, diffuse_irradiance_from_mutiple_reflection_local_df], axis=1)
        # Acquire Diffuse Irradiation from Mutiple Reflection into Data Frame
        diffuse_irradiation_from_mutiple_reflection_local_df = pd.DataFrame(diffuse_irradiation_from_mutiple_reflection_local_list)
        diffuse_irradiation_from_mutiple_reflection_global_df = pd.concat([diffuse_irradiation_from_mutiple_reflection_global_df, diffuse_irradiation_from_mutiple_reflection_local_df], axis=1)
    ...
    diffuse_irradiance_from_mutiple_reflection_global_df = diffuse_irradiance_from_mutiple_reflection_global_df.transpose()
    diffuse_irradiance_from_mutiple_reflection_global_df = diffuse_irradiance_from_mutiple_reflection_global_df.reset_index(drop=True)
    diffuse_irradiance_from_mutiple_reflection_global_df.columns = diffuse_irradiance_from_mutiple_reflection_field_list
    # Organize Diffuse Irradiation form Mutiple Reflection into Data Frame
    diffuse_irradiation_from_mutiple_reflection_global_df = diffuse_irradiation_from_mutiple_reflection_global_df.transpose()
    diffuse_irradiation_from_mutiple_reflection_global_df = diffuse_irradiation_from_mutiple_reflection_global_df.reset_index(drop=True)
    diffuse_irradiation_from_mutiple_reflection_global_df.columns = diffuse_irradiation_from_mutiple_reflection_field_list

    # Combine All Transmittance Coefficient Data Frames into Global 
    global_solar_irradiance_df = pd.concat([direct_normal_irradiance_global_df, separator_df, diffuse_irradiance_global_df, separator_df, diffuse_irradiance_from_rayleigh_atmosphere_global_df, separator_df, diffuse_irradiance_from_aerosol_scattering_global_df, separator_df, diffuse_irradiance_from_mutiple_reflection_global_df], axis=1)
    #print(f'\nGlobal Solar Irradiance Data Frame:\n{global_solar_irradiance_df}')
    global_solar_irradiation_df = pd.concat([direct_normal_irradiation_global_df, separator_df, diffuse_irradiation_global_df, separator_df, diffuse_irradiation_from_rayleigh_atmosphere_global_df, separator_df, diffuse_irradiation_from_aerosol_scattering_global_df, separator_df, diffuse_irradiation_from_mutiple_reflection_global_df], axis=1)
    ...
    new_global_solar_irradiance_df = pd.concat([building_fields_table_df, date_table_df, julian_date_table_df, sunrise_table_df, sunset_table_df, global_solar_irradiance_df], axis=1)
    #print(f'new_global_solar_irradiance_df')
    new_global_solar_irradiation_df = pd.concat([building_fields_table_df, date_table_df, julian_date_table_df, sunrise_table_df, sunset_table_df, global_solar_irradiation_df], axis=1)
    #print(f'new_global_solar_irradiation_df')
    new_global_building_solar_analysis_df = pd.concat([building_fields_table_df, date_table_df, julian_date_table_df, sunrise_table_df, sunset_table_df, global_building_solar_analysis_df], axis=1)
...
def loading_and_prepareing_data_for_qgis(attribute_table_csvfile_name, global_solar_building_analysis_csvfile_location, globla_solar_daily_irradiance_analysis_csvfile_location):
    ...
    for julian_date in range(1, end_julian_date):
        solar_daily_irradiance_analysis_csvfile_local_name = f'Solar_Irradiance_Gov_Building_Day{julian_date}'
        ...
        global_hourly_irradiance_field_list = []
        ...
        for uid in attribute_table_csv_df['uid']:
            ...
            start_time = int(global_solar_building_analysis_df.iloc[julian_date-1, 4])
            stop_time = int(global_solar_building_analysis_df.iloc[julian_date-1, 5])
        ...
        global_hourly_irradiance_df = pd.DataFrame([])
        ...
        for hour in range (start_time, stop_time+1):
            ...
            # Writing Field
            global_hourly_irradiance_field = f"{hour_time}_00"
            global_hourly_irradiance_field_list.append(global_hourly_irradiance_field)

        #uid = attribute_table_csv_df.loc[0, 'uid']
        n = 0
        for uid in attribute_table_csv_df['uid']:
            n+=1
            print(f'Julian Date: {julian_date}, {n/10.17:.3f}%')
            local_date_table_list        = []
            local_hourly_irradiance_list = []
            ...
            local_date_table_list = global_solar_building_analysis_df.iloc[julian_date-1,2:4 ]
            ...
            for hour in range (start_time, stop_time+1):
                ...
                local_direct_normal_hourly_irradiance = global_solar_building_analysis_df.loc[julian_date-1, f"Direct Normal Irradiance-{hour_time}:00"]
                local_diffuse_hourly_irradiance = global_solar_building_analysis_df.loc[julian_date-1, f"Diffuse Irradiance-{hour_time}:00"]
                local_hourly_irradiance = local_direct_normal_hourly_irradiance + local_diffuse_hourly_irradiance
                local_hourly_irradiance_list.append(local_hourly_irradiance)

            local_hourly_irradiance_df = pd.DataFrame(local_hourly_irradiance_list)
            global_hourly_irradiance_df = pd.concat([global_hourly_irradiance_df, local_hourly_irradiance_df], axis=1)
        ...
        global_hourly_irradiance_df = global_hourly_irradiance_df.transpose()
        global_hourly_irradiance_df = global_hourly_irradiance_df.reset_index(drop=True)
        global_hourly_irradiance_df.columns = global_hourly_irradiance_field_list

        new_global_hourly_irradiance_df = pd.concat([global_uid_df, global_date_table_df, global_hourly_irradiance_df], axis=1)

        new_global_hourly_irradiance_df.to_csv(global_solar_daily_irradiance_analysis_csvfile_name, index=False)
        
        print(f'Daily Irradiance CSV file name: {global_solar_daily_irradiance_analysis_csvfile_name}')
        print(F'Julian Date: {julian_date} FINISHED...')
